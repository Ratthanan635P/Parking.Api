...
namespace Parking.TesT
{
	...
	public class CalculateParkingPrice
	{
		...
		private long[,] RangeTax = new long[,] { { 5,0, 50000000, 200000000, 1000000000, 5000000000,0 }
		...
		private double[,] RangeCalTax = new double[,] { { 5,0.003, 0.004, 0.005, 0.006, 0.007,0 }
		...
		private void TaxCalculationFarm()
		{
			...
			else if ((SumPropertyValue > RangeTax[i, 5]) && (SumPropertyValue <= RangeTax[i, 6]))
			{
				j = 1;
				OrderCalculate OrderCalculate1 = new OrderCalculate();
				double sum = RangeTax[i, j + 1] * RangeCalTax[i, j];
			...
			else if (SumPropertyValue > RangeTax[i, 6])
			{
				j = 1;
				OrderCalculate OrderCalculate1 = new OrderCalculate();
				double sum = RangeTax[i, j + 1] * RangeCalTax[i, j];
				...
				OrderCalculate OrderCalculate5 = new OrderCalculate();
				sum = (RangeTax[i, j + 1] - RangeTax[i, j]) * RangeCalTax[i, j];
				OrderCalculate5.Deduction = RangeTax[i, j + 1] - RangeTax[i, j];
				OrderCalculate5.TaxMoney = sum;
				OrderCalculate5.TaxRate = RangeCalTax[i, j];
				OrderCalculatees.Add(OrderCalculate5);
				TaxAll.Add(sum);
				j = 6;
				OrderCalculate OrderCalculate6 = new OrderCalculate();
				double ansValue = SumPropertyValue - RangeTax[i, j];
				sum = ansValue * RangeCalTax[i, j];
				OrderCalculate6.Deduction = (long)ansValue;
				OrderCalculate6.TaxMoney = sum;
				OrderCalculate6.TaxRate = RangeCalTax[i, j];
				OrderCalculatees.Add(OrderCalculate6);
				TaxAll.Add(sum);
			}
			var SumTax = 0.00;
			foreach (var item in TaxAll)
		...
		private void TaxCalculationHomeLand()
		{
			int i = 2;
			int j = 1;
			if ((SumPropertyValue >= RangeTax[i, 1]) && (SumPropertyValue <= RangeTax[i, 2]))
			...
			else if (SumPropertyValue > RangeTax[i, 4])
			{
				j = 1;
				OrderCalculate OrderCalculate1 = new OrderCalculate();
				double sum = RangeTax[i, j + 1] * RangeCalTax[i, j];
		...
		private void TaxCalculationBuild()
		{
			int i = 3;
			int j = 1;
			if ((SumPropertyValue >= RangeTax[i, 1]) && (SumPropertyValue <= RangeTax[i, 2]))
